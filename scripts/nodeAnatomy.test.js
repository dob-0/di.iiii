...
describe('the node anatomy manifest', () => {
    ...
    it('keeps the hand-kept feed places true — the file exists and the editor still mounts it', () => {
        // Feeds are mounted by LiveFeeds, which the editor (and /out) mounts —
        // so a feed counts as mounted when LiveFeeds carries it and the editor
        // still mounts LiveFeeds.
        const editor = read('src/raw/components/RawEditor.jsx')
        const liveFeeds = read('src/raw/components/LiveFeeds.jsx')
        expect(editor, 'RawEditor should mount <LiveFeeds').toContain('<LiveFeeds')
        for (const [id, feed] of Object.entries(FEED_PLACES)) {
            expect(NODE_TYPES[id], id).toBeTruthy()
            expect(read(feed.file), feed.file).toContain(feed.symbol)
            expect(`${editor}\n${liveFeeds}`, `RawEditor or LiveFeeds should mount <${feed.symbol}`).toContain(`<${feed.symbol}`)
        }
    })
})
