...
- **Local podman test stack**: full Docker stack now runs on this machine via podman 5.8 (`podman compose up --build -d`, port 8080; docker-compose v5 binary installed at `~/.docker/cli-plugins/`, rootless `podman.socket` enabled, `.env` from example). All 9 smoke checks pass — parallel test bed for cPanel prod.
