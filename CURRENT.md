...
- **Follow (one space on two installs) is on dev+staging and carries NO assets yet** — a followed scene shows a grey wall where an image or model is (`di follow` says so); no warning yet when op retention drops something uncarried (it drifts in silence); the internet case is the same URL, untested, needs a throwaway space on a tier — owner's call.
- **Tools room, guest mode and the padlocked name are on dev/staging, NOT prod** — `main` untouched, promotion is the owner's word. aylmo runs a branch build (`0.4.5-follow.14`) at https://local.thedi.studio; port 443 comes from setcap on the node binary, which a Node upgrade wipes.
...
- **Naming is guarded now, and three calls are the owner's.** `wcc` meant a coded microsite, a space, a URL prefix, a build chunk and a script; the halves that could move did (`src/wccSite/`, `space:promote`, `gsap-vendor`), the seam that cannot is written at `src/works/works.js`. The general case is closed by two guards: `shared/reservedSegments.cjs` (one reserved-word list, was four that disagreed) and `npm run docs:paths` (every repo path a doc names must resolve — folded into `docs:ai:check`). Still open: the privacy-policy project at `main/privacy` is unreachable at its own URL because `privacy` is a reserved route and reserved words are enforced on slugs but not ids; `LIVE_API_URL` means staging in six scripts and production in two (a migration — the GitHub secret name must survive it); the `Keeper` node label is br_id_ge's ritual word on a shipped generic LLM client, exempted by `vocabulary.md` as dev-local, which stopped being true when it shipped.
