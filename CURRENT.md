...
- the lighting desk grew a content model after a field audit (looks/layers, fan, library import, sACN, palettes), a cue can fire a look, the portability plan is written, and a look can now fan across the ROOM (x/y/radial/angle) with Line sweep / Radar / Grid starters on an arrange stage that no longer hits an edge
...
- **Lighting desk (`/light`)**: local runtime only, 404 on a hosted tier by design — verify on a local stack, and note the edge answers 200 with the SPA there (proxying `/light*` to Node would let the guard's own 404 show; the clients now require JSON). A map cue can fire a look or a scene. A look's phase fan can read the stage arrangement, not just patch order (#350) — Line sweep, Radar and Grid are one-press starters and the arrange world is ±1000 with a backdrop that tiles. `LIGHTING_SHOW_PORTABILITY.md` has the plan for making the show travel — needs the owner's yes before the migration starts (fixture identity by index, the show as a space file not a project-doc key, the club's 588 scenes staying put). Owed, in `LIGHTING_DESK_DESIGN.md` order: cue lists with tracking, the drawn operator surface, a clock with visible phase and nudge, timecode/Link/OSC-in; media playback (video/pixel-mapped visuals) is a bigger lane, not started. Also owed: a real ENTTEC widget on Linux (serial proven on a pty only); the club machine still runs the old standalone build (`~/artnet-desk` syncs `desk/` from here).
