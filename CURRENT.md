...
- **`dev`→`main` promoted and DEPLOYED 2026-09-06 07:00 (#386 — the front door's View mode orbits/zooms/opens doors, #384; every `/spaces` card paints, #383: previews held an SSE stream each and six sockets per origin walled off card 7+, plus the slot freed on iframe `load` not paint; the network field in DEPTH on paper, #382, synced local→staging→prod; dev server sends the font CORS header, #381. Earlier the same night #379 #376 #372 #369). Prod and staging serve the same bundle; zero console errors on `/`, `/wcc`, `/spaces`, `/network`; prod `/spaces` 10/10 cards painted in 20 s; the dragged view-mode frame differs from the still one. Board clear: zero open PRs. Data settled by the owner: `algovrithm` preview cleared, `dilijan` public, `hosq` private (local only), `algovrithm`+`azd` synced to staging. BEHIND PRs land as one batch: memory `feedback-batch-land-behind-prs`.**
...
- **Naming audit done (4 agents), fixes split**: PR #281 (bare `br_id_ge` / `Beyond Form` labels — MERGED and on prod 2026-09-06 with #289's measured landing pass) and **Sign in with Telegram is LIVE on prod** (server #282, button #370, bot `/login` on di-bo master rsynced 2026-09-06; `providers` answers `telegram:true`, bot `diiii111bot`). Not yet walked by a person: DM `/login`, tap, land signed in. No staging bot exists — prod-first was the owner's call. Trap for next time: a new server env var is dead until it is in `docker-compose*.yml` too (#375).
