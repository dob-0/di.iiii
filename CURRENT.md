...
- a PDF import renders every page and then throws; now it returns them
...
- **`dev`→`main` promoted and DEPLOYED 2026-09-06 (#376 — slug addresses, Emily's four fixes via #373, six dependency bumps via #374, compose passes the Telegram vars via #375; earlier the same night #372 and #369). Prod and staging serve the same bundle, zero console errors on `/` `/wcc` `/open` `/beyond_form` `/network` at 1440@2x and 390@3x. The board is clear: zero open PRs; #170 closed (eslint 10 park condition never fired); Emily's #180/#254/#367 closed with replies; 144 merged remote branches deleted; 5 finished worktrees swept. Owed: one human PDF import in Studio after pdfjs 6.3. BEHIND PRs land as one batch: memory `feedback-batch-land-behind-prs`.**
