...
| `npm install` ERESOLVE after bumping vite to 8 | `vite-plugin-restart`'s peerDependency range caps at `vite@^7`, stale metadata — plugin only uses stable `config`/`configResolved`/`configureServer` hooks | `npm install --legacy-peer-deps` is safe here; reviewed plugin source, no vite8-incompatible API used | `package.json` |
