...
- ~~Live DB renames owed on PROD~~ **DONE 2026-09-01, and the block was imaginary**: `PATCH /api/spaces/:id` with `PROD_API_TOKEN` does labels, owners and project titles — no ssh needed. Prod now reads Works / RecordAR / WCC Exhibition / "Everything made here" (`di.ii` had been the flagship's og:title for weeks), and the five ownerless prod spaces have an owner. **Two owner accounts exist** — check which before anything account-bound.
...
- **`the-light-put-back` is LIVE on staging but PRIVATE** — a new work (14 laser photographs, MOCT × MECHATRONICA) arrived as a space, not repo code. Three owner calls: make it public (`isPublic` is approval-gated, and they are Davit Nersisyan's photographs); re-cut its 4.3 MB inlined page as space assets before prod (`dii-space-weight-audit`); the platform's STAGING badge sits on top of that page's own transport bar, bottom-left.
- **`space:new` → `space:code-push` still does not produce a visible page** — three script bugs are fixed, but a fresh space with no `publishedProjectId` opens its scene regardless of what its project holds. `PATCH /api/spaces/:id {publishedProjectId}` is a fourth manual step nothing performs. Full recipe: memory `reference-dii-space-code-push`.
