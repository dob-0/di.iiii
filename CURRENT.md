...
- **Naming audit done (4 agents), fixes split**: PR #281 (bare `br_id_ge` / `Beyond Form` labels — MERGED and on prod 2026-09-06 with #289's measured landing pass) and PR #282 + #370 (sign in with Telegram: server half AND the web button on prod; bot half merged in di-bo, UNDEPLOYED; `telegram:false` until `TELEGRAM_LOGIN_SECRET` is set in both `.env`s — owner's line, and there is no staging bot). Still the owner's to settle: does `di.i` survive; commons or funnel; does the position name Armenia; `window` vs `panel`; `Raw` (the guard bans the word the product uses everywhere).
