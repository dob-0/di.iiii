...
- **`dev`→`main` promoted and DEPLOYED 2026-09-06 (#379 — a visitor to `/spaces` sees every space, #378; a PDF import returns its pages, #377 — that one was broken since pdfjs 5, `doc.destroy()` never existed; earlier the same night #376 #372 #369). Prod and staging serve the same bundle; zero console errors on `/` `/wcc` `/open` `/beyond_form` `/network`; `/spaces` has ONE: `algovrithm`'s `previewImageAssetId` (1769c1cb…) 404s on prod, staging no longer has the space and local has no pointer — owner's one PATCH (clear it or re-upload), the card already falls back to the live preview. `dilijan` is `isPublic` on prod and now shows on `/spaces` — flip it if the camp space was meant to stay off the front. Board clear: zero open PRs; BEHIND PRs land as one batch (memory `feedback-batch-land-behind-prs`).**
