...
- **All 8 prod spaces are declared.** Engine v6 reads an empty `projects` list as a space-only declaration, so Studio-authored and code spaces can be declared at all; `npm run spaces:audit` walks every one, read-only. Prod and staging agree on every declared field. Also live from another session (`5a34baa3`): `?preview=1` was honoured only by `PublicProjectViewer`, so a space-card thumbnail with no published project embedded the whole editor and showed Enter VR / Enter AR / Mode.
