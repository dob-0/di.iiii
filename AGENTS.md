...
- End every task by **recording**: **summary** (2–4 lines), **changed files** (one-line reason each), **validation** (commands + pass/fail), **risks** (concrete only). This is a written record — it belongs in the session note, PR body, or `CURRENT.md`, **not** pasted into chat. The operator's global `~/.claude/CLAUDE.md` says "no trailing summaries or recaps"; that governs what you *say*, this governs what you *write down*. Where the two ever conflict again, the operator's global file wins for chat-facing output and this file wins for the durable record. (Adjudicated 2026-08-11; the two rules contradicted each other outright and every session resolved it by guess.)
