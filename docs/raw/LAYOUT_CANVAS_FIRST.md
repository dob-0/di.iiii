...
| < 0.18 | **block** | solid `var(--di-black)` rect, 1px `--di-cyan-border`, no text, category colour as a 3px left edge bar | none rendered; wires still terminate at the same coordinates | pan/read topology only; tap selects and the selection bar names it; `◎ Frame` gets you there |
