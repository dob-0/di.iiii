...
- `Raw`
  - experimental recursive node-first lane (absorbed Beta's role 2026-08-06)
...
3. `Studio` and `Raw` consume that shared layer
...
- `Raw` is the strongest node-first surface, but still experimental
...
- experimental node-first UX: start in `src/raw/`
