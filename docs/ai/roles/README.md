...
| [3D/Viewport Engineer](viewport-3d-engineer.md) | VPE | Three.js scene, RawViewport + StudioViewport, objectComponents, XR | CSS layout, nodeRegistry logic, serverXR |
...
- Studio is the main shipped lane; Raw is experimental and where the future is being built
  (the Beta lane was retired 2026-08-06 and absorbed into Raw — `src/beta/` no longer exists).
  Do not ship experimental Raw behavior as Studio's default UX, and do not force a
  "which lane is primary" decision on the landing page ahead of the unification work itself
