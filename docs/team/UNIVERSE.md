...
### Node Engineer
Owns the node-first authoring lane. Explores patterns that may eventually ship in Studio. Works with the node registry, node palette, window layout, and the local workspace.

- Home: `src/raw/`, `src/project/nodeRegistry.js`
...
### Spatial Designer
Composes scenes using the Studio editor or the node canvas. Arranges objects in 3D space, sets transforms, lighting, and camera angles. Authors the spatial experience that gets published as the live space.

- Home: the editor — Studio at `/<space>/studio`, the node canvas at `/<space>/raw`
...
| Node Engineer | `dii-raw-node-authoring` · `dii-feature-routing` |
