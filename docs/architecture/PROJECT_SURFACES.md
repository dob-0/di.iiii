...
| `Raw` | `/<space>/raw` and `/raw` | Experimental lane, forked from Beta (2026-07-19) | Free-form node nesting (no singletons), universal per-node code panel — see `docs/architecture/RECURSIVE_NODE_CORE.md` |
...
- `src/raw/`
  - Raw-specific routes and UI — a fork of `src/beta/` (2026-07-19), same ownership rules apply
...
- keep `Raw` scoped under spaces the same way — no special-casing beyond what Beta already gets
...
than committing up front to "Raw replaces Beta." Whether Raw eventually
