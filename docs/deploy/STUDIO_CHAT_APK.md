...
`public/chat-app/di-studio-chat.apk` — the signed prod build, tracked, deployed
with everything else, and therefore downloadable at

    https://di-studio.xyz/chat-app/di-studio-chat.apk

That address is the one to hand out, and it is what di.bo's `/app` sends (it
sends the URL, so Telegram fetches the file; nothing is copied onto the VPS).
**Copy a new build in whenever the APK changes** — the file in `public/` is not
generated by anything, and a stale one installs over nobody's app because the
version code never moved.
