...
export const WIKI_ARTICLES = [
    ...
    },
    {
        ...
        summary: '/{space}/make/{project} opens a project as one full-screen room with four words under it — add, colour, photo, talk. Same document as Raw, a lid a child can hold.',
        body: [
            'Made for a day camp in Dilijan, where five children aged 10 to 17 were building 3D rooms on their own phones. Raw is a workshop bench, and on a 390px screen it is eight stacked window bars and a node graph at 34% zoom — with the thing being made nowhere on the screen. The toybox is the same project with a different lid: the room fills the screen and is tappable, and four words sit under it, Armenian first and English beneath. Add puts one of five shapes on the ground in front of you; colour paints what you tapped, from nine; photo takes a picture from the phone and stands it in the room; talk opens the chat, here or with everyone in the space.',
            'Nothing new is stored. It is the same project document, the same op layer and the same renderer as Raw, so a mentor opening /{space}/raw/projects/{project} sees every object a child made, signed by name — the toybox asks for a first name once, and that is the name on the work.',
            'One address shape only: /{space}/make/{project}. There is no list page and no default — a child follows a link a mentor wrote down, because a lane that guessed which project you meant could open the wrong child\'s room. A bare /make is not the toybox and opens nothing useful. It sits behind the same gate as Raw, so an invite that opens the space opens its toybox too.'
        ],
