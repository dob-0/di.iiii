...
export const WIKI_ARTICLES = [
    ...
    },
    {
        ...
        body: [
            ...
            'Signed in, the Spaces page (/studio) shows exactly these shelves: Open Space · Your sandbox · Your spaces — so the answer to “where am I, what’s mine” is always the page itself. Admins additionally see guest sandboxes collapsed into one row with a count and a “Sweep expired” action.',
            'Signed out or as a guest, that same page is simply the public spaces: one grid holding every space open to you, all of it visible on arrival, the Open Space one card among the others, the ones with something to show first. Your private sandbox is a single line under the grid rather than a card, since it is a scratch place of your own and not somewhere to visit. Click any public space’s picture to make it interactive right there in its card — walk around, look, without leaving the page — a plain “Open ↗” stays visible if you want the full page; click another picture (or scroll the live one away) and it hands the room back for that one.',
            ...
            'For a live jam — an event, a workshop, a projected wall — share the short link diiii.xyz/open_jam (or turn it into a QR code). It opens the shared Open Jam directly, and a one-line welcome shows first-timers how to add their visual. Anyone who scans can drop in an image, video or 3D model on the spot, no account needed.',
    ...
    },
    {
        ...
        body: [
            'diiii.xyz/open_jam/scene opens the shared Open Jam as a place you are in, rather than as an editor with most of it switched off. You arrive standing at eye height. Drag a thumb across the right of the screen to look around, and the left to walk.',
            ...
            'Share, next to the ＋, hands the room to somebody else: on a phone it opens your own share sheet, so a text or a message app is one tap away; everywhere else it copies the plain diiii.xyz/open link to the clipboard. Getting a second person in takes one tap, not spelling out a URL.',
            ...
            'The editor is unchanged and still lives at diiii.xyz/open_jam. The two are the same jam — whatever is made in one shows up in the other, live — so a laptop can be doing the careful work while the phones in the space keep adding.'
        ],
    ...
    },
    {
        ...
        tags: ['light', 'lighting', 'dmx', 'artnet', 'enttec', 'desk', 'scene', 'show', 'stage', 'performance', 'local', 'di up', 'lan', 'phone', 'rig', 'fixture', 'marker', 'studio', 'index', 'positions'],
        updated: '2026-09-21'
    },
    {
    ...
    },
    {
        ...
        body: [
            'Projection opens a mapping, at diiii.xyz/{space}/map/{project}. A mapping is a project whose contents are other projects: a list of surfaces, each one a four-cornered shape on the wall with something playing inside it.',
            ...
            'Cues are not only the projection tool’s. They belong to the project, so the same cues appear in Studio as a small row in the control panel and the same number keys fire them from inside the 3D scene — on a phone the row sits above the bottom bar, one finger away. Either way the wall and the lighting desk do exactly what they do when the key is pressed at the mapping desk.',
    ...
    },
    {
        ...
        body: [
            ...
            'A few spaces share their address with a piece that is part of diiii.xyz itself — /wcc is the WCC exhibition’s own landing page, and the wcc SPACE, with its eleven artist projects, lives one step deeper. Where that happens, the space card no longer points at the piece: its picture, its live frame and its Live link all open the space’s published project, so the card is always a door to the space it names. The piece keeps its own address; nothing about /wcc itself changed. On an offline copy of di.iiii, where those pieces are left out to keep the download small, such a card used to say “not in this copy” even though every project it holds was sitting on the machine — now it opens them.',
    ...
    },
    {
        id: 'algovrithm',
        category: 'Spaces & access',
        title: 'algovrithm — a VR piece written in code',
        summary: 'algovrithm is a VR piece written in three.js/R3F code rather than made in Studio. /algovrithm introduces it; /algovrithm/scene is the piece.',
        body: [
            'algovrithm is a WebXR piece that lives inside di.iiii’s own code, as WCC does. Who may see it is decided the way it is for any space — the server says whether the algovrithm space is public — but that space holds no projects, and its address never opens one: /algovrithm is always the piece’s front door. What you walk into is not a project you edit in Studio. It is code, living in src/algoVrithm/, so it can do things the object model does not express (generative geometry, per-frame math, custom shaders).',
            { list: [
    ...
    },
    {
        ...
        body: [
            ...
            'It does not need admin rights, and it does not ask for a password. It runs as a single ordinary program, and if there is no suitable Node it quietly fetches its own rather than sending you away. It always installs as that ordinary program, never as a container, so the surfaces that talk to your own tools (the agent board, a Claude installed on this computer) can reach them.',
            'A local space can be linked to one on diiii.xyz: di link connects the two, and di sync compares them — what each side has that the other has not, and whether sending work either way would be safe or refused, as it is when both sides have changed. It only looks: it writes nothing on either side. To carry work between them, use di save and di open, di backup, or Save to file on the Spaces page.',
    ...
    },
    {
        ...
        body: [
            ...
            'di up --lan --guests turns auth on for everyone except the person at the machine. A visitor arrives as a guest exactly as they would on diiii.xyz: their own sandbox room and the shared open space, editor there and nowhere else. Your spaces are not listed, a private one answers 403, and so does the admin page.',
    ...
    },
    {
        ...
        body: [
            ...
            'A project’s files travel too, both ways: an image, a video or a model added to a project on either machine is carried to the other, one file at a time, and checked against its name before it is kept. Two kinds do not travel yet — files placed straight in the space’s own scene rather than in a project, and older files added before files had checkable names. Both stay on the machine where they were added; di follows counts the older ones, and adding one again sends it.'
        ],
        tags: ['follow', 'invite', 'collaboration', 'local', 'two machines', 'sync', 'files', 'images', 'models'],
        updated: '2026-09-23'
    },
    {
        id: 'keeper-node',
    ...
    },
    {
        ...
        body: [
            ...
            'Both rigs need a di.iiii running on your own machine, for different reasons. The desk only exists on a local di.iiii (`di up`, or npm run dev) — a hosted diiii.xyz has no desk in it, and the panel says so rather than going quiet. A vizzz box speaks http, and a https page is not allowed to reach it — the panel says that too. On a hosted tab the node is honest and idle; on your own machine it lights the room.'
        ],
    ...
    },
    {
        ...
        body: [
            'A space is a place. If you have a real one — a factory hall, a theatre, a foyer — you can walk it with a phone and have it become a space you can stand in. Press "Scan a place" on the new-space form and the camera opens on the space it just made, at diiii.xyz/{space}/scan.',
