...
export const WIKI_ARTICLES = [
    ...
    },
    {
        ...
        body: [
            ...
            'That motion used to start only when a visitor pressed Walk / Fly, so the first screen anyone saw was a still photograph of a room that came alive one click later. A published scene now moves from the frame it opens on, and proximity — an object that comes up as you approach it — works there too. The editor still holds perfectly still while you are placing things; only the published page moves. Fog behaves the same way in both views now, and so do the grid’s colours and spacing and the render settings (exposure, tone mapping, antialiasing): what you author is what a visitor sees, before and after the click.',
