...
export const WIKI_ARTICLES = [
    ...
    },
    {
        ...
        body: [
            ...
            { list: [
                ...
                'Starting from nothing: a blank Raw workspace shows a “Make me a scene” button in the canvas’s lower band (out of the double-click zone). It builds a room with a light, a cube, a colour wired into that cube, and an empty Model node waiting for your own file — plus a note giving the moves in plain words. It only offers itself on a truly blank desk at the top level: inside a container the empty state stays yours (a stray double-click used to inject the whole demo INTO the container being filled); the ⋯ menu still offers it deliberately, anywhere. It is there because a blank workspace opens with no toolbar at all, so the ⋯ menu (which also offers it) does not exist for the person most likely to want it.',
                'The Geo is the plain container — TouchDesigner’s Geometry COMP, by name. It arrives empty but is visibly a place (a faint floor tile marks its footprint), you enter it and collect what you need — objects, models, Lights — and everything renders inside it and travels with it. It adds nothing of its own — no shell, no rules — and it gives out what it collects: a Geometry socket carrying everything standing in it as one shape, so Geo → Merge → Constructor composes collected scenes, and a Geo standing inside a Geo carries through. When in doubt, build in a Geo. A Geo stands on the floor, and in the room a click picks up the whole Geo — drag it or set its Position to part two geos; enter the Geo to handle one thing inside. A place shows only what stands IN it: objects made in Studio’s Create window live in the top room and never haunt a container’s inside.',
                'A Light can stand somewhere now: placed inside any container it is a real point light with a small glowing marker — colour, intensity and position of its own. At the top level it keeps its old job untouched: the room’s ambient and directional settings.',
                'The Camera is the authored eye. Placing one never steals the view: it stands in the room as a small housing until the ● toggle on its card marks it as the eye for this level. Marked, the room is seen through it — Position, Look At and FOV are inputs like any other, so a wire can move the shot — its housing disappears, and orbiting is off because the shot is authored. Unmark (or delete) it to look around freely again.',
                'The projector cable: /out. Every project has an output address — /{space}/raw/projects/{id}/out — that renders just the room, read-only, zero chrome, following every edit live. A space\'s own canvas has /{space}/raw/out (same browser only — a local canvas lives in that browser). Add ?scope= to output a container\'s room; mark a Camera ● there and the output holds the authored shot. Open it on the show machine, press F11, walk away.',
                'The Constructor is the first node made of nodes: a container that wears whatever shape the nodes inside it build. Place parts inside and it wears them — wire an Out door only when you want to say “exactly this, nothing else”. See “The Constructor: a node made of nodes”.',
                                'In the room, touch works the way you expect: click an object to select it, click empty floor to deselect, drag to move it (the camera holds still), hold Shift while dragging to lift it, Ctrl/Cmd+D to duplicate the selected node.',
                'The desk is clear — always. Cards on flat paper, nothing behind them. The room is a view you open: the Scene window (drag its corner glyph to size it), the Room button or the palette\'s Room command for fullscreen, and /out for a whole display. Fullscreen survives walking through doors: each door swaps which room fills the screen.',
                ...
                'The Monitor is the desk\'s viewer window: wire any texture into its Source — a Webcam\'s Frame — and watch it live while you keep wiring, TouchDesigner\'s operator viewer as a window. It only watches; rooms have the World window, the Room button and /out.',
    ...
    },
    {
        ...
        body: [
            ...
            { list: [
                ...
                'Enter it. Place a Cube or a Sphere or a Plane — it appears in the room behind your cards as you place it, and the Constructor outside is already wearing it. No wiring needed: everything you place inside contributes, like TouchDesigner’s flags.',
    ...
    },
    {
        ...
        body: [
            ...
            'A first visit opens on a clean, empty room — one sentence, one offer. “Make me a scene” builds a worked example (a room, a light, a cube with a colour wired in) the moment you ask for it, and only then: choosing the demo is your act, not the product’s.',
