...
export const WIKI_ARTICLES = [
    ...
    },
    {
        ...
        body: [
            ...
            'A local di.iiii wears a green border and a small LOCAL badge in the corner, with the address it is answering on. The second tier wears an amber one — DEV at dev.diiii.xyz, STAGING at staging.di-studio.xyz, the same tier under either name. The live site wears nothing — so if there is no border, you are on diiii.xyz and anyone can see what you do next. It is there because two di.iiii that look identical are two di.iiii you will eventually confuse.',
    ...
    },
    {
        ...
        body: [
            ...
            { list: [
                ...
                'See, at the top, shows the node working, by what it makes: a picture operator\'s picture, the Cube itself, a Webcam\'s own window, a ten-second trace of an Oscillator, a colour\'s history, the words a text node gives. It is sized to what it actually shows — a picture or a shape gets the wide 16:9 band, a number or a swatch gets a short strip — so a Colour or an Oscillator does not push its own settings off a phone screen. Fold it away with its arrow when you want the room.',
                'In, on the left, is every setting and every input in one list, with the same fields as the Inspector, one line per row: label, its field or live value, and a small dot. A wired input reads its value and, right on the same line, "← " the card it comes from — press it to go there, or × to unplug. The dot takes a wire (drag one from a card inside and drop it on the dot, or press the dot and pick from what stands inside); its colour names the type — press or hover it for the word.',
                'Out, on the right, is what the node gives right now and where every wire from it goes, also one line: label, value, "→" the card it reaches. Press that to walk there, wherever it is. Its dot feeds a card inside, so what you build inside can read the node too.',
            ...
            'The head says where you are, once: back, the node\'s name (press it to rename), and a quiet chip for where it runs — "here" for almost everything, a machine\'s name only for a picture operator pinned to one. It no longer repeats the node\'s own name as a second word next to it, and a row\'s field never spells out its own type in words next to a label that usually already says it (Colour, Size) — that lives in the dot alone now. Below 1100 pixels Out folds under In; on a phone the frame is one column, See stays at the top while you scroll, and In · Out · Made of · Canvas sit at the bottom under your thumb — the settings list starts above the fold.',
            'Going inside a panel node whose window you had closed opens the window again as well, so it is there when you come back out. The old “Code — stored, not run” box is gone: nothing ever ran it. A script that really runs lives in Made of. An empty inside canvas is a short line under the frame\'s own chrome now, with a "+" to place the first node, not a big empty grid.'
        ],
