...
export const WIKI_ARTICLES = [
    ...
    },
    {
        ...
        body: [
            ...
            'Whether the button is there at all: a published page offers Walk / Fly on any room, however it was built. A project made of nodes and nothing else used to be the exception — walk/fly could only draw objects, so it would have dropped the visitor into an empty version of the room they were already looking at, and the button was hidden rather than lie. Walk mode now draws the node bodies too, so that exception is gone: a room built entirely in the node editor can be stepped into, and VR and AR come with it, since a headset session is entered from inside walk/fly.',
