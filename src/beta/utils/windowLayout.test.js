...
// Regression guard for audit batch 2: Beta filtered panels by visibility only,
// so every world node in the document kept a live WebGL context mounted in
// every scope (browsers cap ~16, then the kill/remount loop freezes the tab)
// and one scope's windows floated over another's graph. Raw fixed this first.
describe('selectMountedPanelNodes', () => {
    ...
    it('never mounts a whole nested chain at once — the WebGL context cap is ~16', () => {
        expect(selectMountedPanelNodes({ nodes: deepChain, isPanel, currentScopeId: null }).length)
            .toBeLessThan(16)
    })
