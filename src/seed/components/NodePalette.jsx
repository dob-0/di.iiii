...
export default function NodePalette({
    ...
    return (
        <div
            className="seed-node-palette-backdrop"
            role="presentation"
            onMouseDown={(event) => {
        ...
        >
            <div
                className="seed-node-palette"
                role="dialog"
                aria-modal="true"
                aria-label="Create node"
                style={{ left: pos.x, top: pos.y }}
            >
                <div className="seed-node-palette-input-row">
                    <input
                        ref={inputRef}
                        className="seed-node-palette-input"
                ...
                {definitions.length > 0 ? (
                    <ul ref={listRef} className="seed-node-palette-list">
                        {definitions.map((definition, index) => (
                            <li key={definition.id}>
                                <button
                                    type="button"
                                    className={`seed-node-palette-item${index === activeIndex ? ' is-active' : ''}`}
                                ...
                                >
                                    <span className="seed-node-palette-item-title">
                                        <strong>{definition.label}</strong>
                                        <span>{definition.id}</span>
                                    </span>
                                    {definition.authoringOnly && (
                                        <span className="seed-node-palette-tag" title="Placeable and editable, but doesn't compute or render anything yet">
                                            authoring only
                                        </span>
                                    )}
                                </button>
                    ...
                    <div className="seed-node-palette-empty">no match</div>
                )}
            </div>
        </div>
    )
