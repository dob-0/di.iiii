        ...
        >
            ...
            {selected && (
                <Html position={[0, 1.8, 0]} center>
                    <span className="seed-selection-pill">{entity.name}</span>
                </Html>
            )}
        </group>
    )
        ...
        >
            ...
            {selected ? (
                <Html position={[0, 1.5, 0]} center>
                    <span className="seed-selection-pill">{node.label}</span>
                </Html>
            ) : null}
        </group>
    )
...
function SceneContent({
    ...
    return (
        <>
            <color attach="background" args={[getSeedWorldBackgroundColor(document, graphContext, { scopeId, worldNode })]} />
            <ambientLight
...
export default function SeedViewport({
    topInset = 0,
    document,
    selectedEntityId,
    onSelectEntity,
    ...
    const handleViewportDoubleClick = (event) => {
        if (event.target?.closest?.('.seed-cursor-layer, .seed-cursor-marker, .seed-selection-pill')) return
        onWorldDoubleClick?.({
            point: [0, 0, 0],
    ...
    return (
        ...
        >
            {showEmptyHint && isEmpty ? (
                <div className="seed-viewport-empty-hint">
                    <div className="seed-viewport-empty-stage" aria-hidden="true">
                        <div className="seed-viewport-empty-grid" />
                        <div className="seed-viewport-empty-crosshair" />
                    </div>
                    <div className="seed-viewport-empty-panel">
                        <span className="seed-window-kicker">World</span>
            ...
            <div className="seed-cursor-layer">
                {Object.values(cursors).map((cursor) => (
                    <div
                        key={cursor.socketId || cursor.userId}
                        className="seed-cursor-marker"
                        style={{
