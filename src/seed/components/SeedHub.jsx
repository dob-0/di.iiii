...
import { buildSeedProjectPath, navigateToSeedPath } from '../utils/seedRouting.js'
import { GUIDE_AUDIENCES } from '../utils/seedGuide.js'
import SpaceSyncPanel from '../../components/SpaceSyncPanel.jsx'

export default function SeedHub({ spaceId = DEFAULT_PROJECT_SPACE_ID }) {
    ...
    const openProject = (projectId) => {
        navigateToSeedPath(buildSeedProjectPath(projectId, spaceId))
    }
    ...
    return (
        <main className="seed-hub">
            <div className="seed-hub-layout">
                <header className="seed-hub-header">
                    <div className="seed-hub-wordmark">
                        <span className="seed-hub-di-sq" />
                        <span className="seed-hub-di-sq" />
                        <span className="seed-hub-di-sq" />
                    </div>
                    <h1 className="seed-hub-title">di.iiii seed</h1>
                    <p className="seed-hub-tagline">space · {spaceId}</p>
                </header>

                <section className="seed-hub-onboarding" aria-label="Seed onboarding">
                    <div className="seed-hub-onboarding-copy">
                        <span className="seed-window-kicker">First Landing</span>
                        <h2>Choose a path.</h2>
                        <p>Look first, or build first.</p>
                    </div>
                    <div className="seed-hub-onboarding-grid">
                        {GUIDE_AUDIENCES.map((audience) => (
                            <section key={audience.id} className="seed-hub-onboarding-card">
                                <div className="seed-hub-onboarding-mark" aria-hidden="true">
                                    <span>{audience.glyph}</span>
                                </div>
                                <span className="seed-window-kicker">{audience.label}</span>
                                <h3>{audience.title}</h3>
                                <div className="seed-hub-onboarding-chip-row">
                                    {audience.tags.map((tag) => (
                                        <span key={tag} className="seed-hub-onboarding-chip">{tag}</span>
                                    ))}
                                </div>
                                <ol className="seed-hub-onboarding-steps">
                        ...
                        <section className="seed-hub-onboarding-card">
                            <div className="seed-hub-onboarding-mark" aria-hidden="true">
                                <span>↔</span>
                            </div>
                            <span className="seed-window-kicker">Workflow</span>
                            <h3>Space → Seed → Studio</h3>
                            <div className="seed-hub-onboarding-chip-row">
                                <span className="seed-hub-onboarding-chip">space</span>
                                <span className="seed-hub-onboarding-chip">project</span>
                                <span className="seed-hub-onboarding-chip">publish</span>
                            </div>
                            <ol className="seed-hub-onboarding-steps">
                ...
                <div className="seed-hub-create-row">
                    <input
                        ref={titleInputRef}
                        className="seed-hub-title-input"
                        value={title}
                        onChange={(event) => setTitle(event.target.value)}
                        placeholder="project title"
                        onKeyDown={(e) => e.key === 'Enter' && !isBusy && handleCreate()}
                    />
                    <button type="button" className="seed-hub-create-btn" onClick={handleCreate} disabled={isBusy}>
                        new
                    </button>
                    <label className="seed-hub-import-btn">
                        <input type="file" accept=".zip,.json,application/zip,application/json" onChange={handleImport} />
                        import
                    </label>
                </div>

                <div className="seed-hub-projects">
                    {projects.length ? (
                        <ul className="seed-project-list">
                        ...
                        <p className="seed-hub-empty">{status}</p>
                    )}
                </div>

                <SpaceSyncPanel spaceId={spaceId} />

                <footer className="seed-hub-footer">
                ...
                {importWarnings.length ? (
                    <div className="seed-hub-warnings">
