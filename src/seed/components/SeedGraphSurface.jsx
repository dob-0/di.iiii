...
export default function SeedGraphSurface({
    topInset = 0,
    nodes = [],
    edges = [],
    selectedNodeId = null,
    ...
    const shouldStartPan = (event) => {
        const target = event.target
        if (target?.closest?.('.seed-graph-zoom-controls')) return false
        if (event.button === 1) return true
        if (event.button !== 0) return false
        return !target?.closest?.('.seed-graph-node-card')
    }
    ...
    return (
        ...
        >
            <div className="seed-graph-zoom-controls">
                <button type="button" aria-label="Zoom out" onClick={() => updateZoom(zoom - GRAPH_ZOOM_STEP)}>-</button>
                <span className="seed-graph-zoom-value">{Math.round(zoom * 100)}%</span>
                <button type="button" aria-label="Zoom in" onClick={() => updateZoom(zoom + GRAPH_ZOOM_STEP)}>+</button>
            </div>
            {nodes.length === 0 ? (
                <div className="seed-empty-state" style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', color: '#aaa', pointerEvents: 'none' }}>{emptyHint}</div>
            ) : null}
            <div
                className="seed-graph-stage"
            ...
            >
                    ...
                    {nodes.map((node) => {
                        ...
                        return (
                            <div
                                key={node.id}
                                className={`seed-graph-node-card${isSelected ? ' is-selected' : ''}`}
                                style={{
                                    position: 'absolute',
                                    left: node.graphX,
                                    top: node.graphY,
                            ...
                            >
                                <header className="seed-graph-node-header">
                                    {activeMarkerTypeIds.includes(node.typeId) && (
                                        <button
                                            type="button"
                                            className={`seed-graph-node-active-toggle${isNodeActive(node) ? ' is-active' : ''}`}
                                    ...
                                    <span className="seed-graph-node-icon" />
                                    <span className="seed-graph-node-label">{node.label}</span>
                                    <span className="seed-graph-node-category">{typeDef?.category || ''}</span>
                                    <span className="seed-graph-node-enter-hint" title="Double-click to enter">›</span>
                                </header>
                                <div style={{ position: 'relative', height: h - HEADER_HEIGHT }}>
                                    {inputs.map((port, idx) => (
                                        <div
                                            key={`in-${port.id}`}
                                            className="seed-graph-port-row seed-graph-port-row--in"
                                            style={{ top: idx * PORT_ROW_HEIGHT }}
                                        >
                                            <span
                                                className="seed-graph-port-dot seed-graph-port-dot--in"
                                            ...
                                            />
                                            <span className="seed-graph-port-label">{port.label || port.id}</span>
                                        </div>
                                    ))}
                                    {outputs.map((port, idx) => (
                                        <div
                                            key={`out-${port.id}`}
                                            className="seed-graph-port-row seed-graph-port-row--out"
                                            style={{ top: idx * PORT_ROW_HEIGHT }}
                                        >
                                            <span className="seed-graph-port-label">{port.label || port.id}</span>
                                            <span
                                                className="seed-graph-port-dot seed-graph-port-dot--out"
