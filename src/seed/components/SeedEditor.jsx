...
import SeedViewport from './SeedViewport.jsx'
...
function BrowserPanelWindow({ node }) {
    ...
    return (
        <div className="seed-browser-panel-window">
            <div className="seed-browser-panel-bar">
                <strong>{title}</strong>
                <span>{url}</span>
            </div>
            <iframe
...
export default function SeedEditor({
    ...
    const hostInspector = (
        <aside className="seed-selection-scaffold" style={{ top: workflowHeight + 'px' }}>
            <PropertyInspector
    ...
    const workspaceTitle = isLocalWorkspace ? 'Blank White Workspace' : (document.projectMeta?.title || 'Seed Project')
    const graphTopInset = chromeVisible ? workspaceTop : 0

    return (
        <main className="seed-editor-shell">
            <header className={`seed-topbar${chromeVisible ? ' is-seeded' : ''}`} ref={topbarRef}>
                {chromeVisible && (
                    <>
                        <div className="seed-topbar-left">
                            <button type="button" className="seed-topbar-back" onClick={() => {
                                navigateToSeedPath(buildSeedProjectsPath(resolvedSpaceId))
                            }}>
                                ← {isLocalWorkspace ? 'Projects' : 'Hub'}
                            </button>
                            <span className="seed-topbar-name" title={workspaceTitle}>{workspaceTitle}</span>
                        </div>
                        <div className="seed-topbar-center">
                            {navStack.length > 1 ? (
                                <nav className="seed-topbar-breadcrumb" aria-label="Node scope">
                                    <button type="button" className="seed-topbar-crumb" onClick={() => handleNavigateToScope(0)}>◈</button>
                                    {navStack.slice(1).map((scopeId, i) => {
                                        ...
                                        return (
                                            <span key={scopeId} className="seed-topbar-crumb-group">
                                                <span className="seed-topbar-crumb-sep">›</span>
                                                <button
                                                    type="button"
                                                    className={`seed-topbar-crumb${isLast ? ' is-current' : ''}`}
                                ...
                                <span className="seed-topbar-location" aria-live="polite">{topbarLocationText}</span>
                            )}
                            {hasWorldNode && (
                                <div className="seed-topbar-windows">
                                    <button
                                        type="button"
                        ...
                        <div className="seed-topbar-right">
                            <button type="button" className="seed-topbar-help-action" onClick={() => setHelpOpen(true)}>
                                Help
                            </button>
                            <div className="seed-topbar-scale-control">
                            ...
                            {surfaceNodeCount > 0 && (
                                <button
                                    type="button"
                                    className={`seed-topbar-node-count${outlinerOpen ? ' is-active' : ''}`}
                            ...
                            <button
                                type="button"
                                className={`seed-topbar-node-count${chatOpen ? ' is-active' : ''}`}
                            ...
                            <div className="seed-topbar-overflow">
                                <button type="button" className="seed-topbar-overflow-btn" onClick={() => setOverflowOpen((v) => !v)}>⋯</button>
                                {overflowOpen && (
                                    <div className="seed-topbar-overflow-menu">
                                        ...
                                        {presence.users.length > 0 && presence.users.map((user) => (
                                            <span key={user.socketId || user.userId} className="seed-user-pill">
                                                {user.userName}
                                            </span>
                                        ))}
                                    </div>
            ...
            {state.loading ? <div className="seed-overlay-message">Loading project…</div> : null}
            {state.loadError ? <div className="seed-overlay-message is-error">{state.loadError}</div> : null}
            {visibleSelection && (
                <button type="button" className="seed-delete-fab" onClick={handleDeleteSelected}>
                    Delete
                </button>
            )}

            <section className={`seed-surface-shell${isWorldOverlay && !isWorldFullscreen ? ' is-world-overlay' : ''}${navStack.length > 1 ? ' is-inside-node' : ''}`}>
            ...
            {hasWorldNode && isWorldFullscreen && (
                <div className="seed-world-fullscreen" style={{ top: `${workspaceTop}px` }}>
                    <SeedViewport
                        topInset={0}
                        document={document}
                        selectedEntityId={surfaceSelectedEntity?.id || null}
            ...
            {hasWorldNode && isWorldOverlay && !isWorldFullscreen && (
                <div className="seed-world-overlay">
                    <SeedViewport
