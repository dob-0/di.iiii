...
function PropertyField({ field, value, onChange, assetOptions = [] }) {
    ...
    if (field.type === 'presets' || field.type === 'modelClips') {
        // Studio-only widgets (multi-key patch / clip registry); Seed skips them
        return null
    }
...
export default function PropertyInspector({
    ...
    if (!sections.length) {
        return <div className="seed-empty-state">{emptyMessage}</div>
    }

    return (
        <div className="seed-property-sheet">
            <header className="seed-property-sheet-header">
                <h4>{title}</h4>
                {subtitle ? <p>{subtitle}</p> : null}
            </header>
            <div className="seed-property-sections-scroll">
                {sections.map((section) => {
                    const sectionValue = values[section.id] || values[section.component] || {}
                    return (
                        <section key={section.id} className="seed-property-section">
                            <h5>{section.label}</h5>
                            <div className="seed-property-grid">
                                {section.fields.map((field) => {
                                    ...
                                    return (
                                        <label
                                            ...
                                            className={`seed-property-field${field.type === 'checkbox' ? ' seed-checkbox-field' : ''}${isFullWidth ? ' seed-full-width-field' : ''}`}
                                        >
