...
// Mock 3D deps before importing SeedEditor to avoid ResizeObserver errors in jsdom
vi.mock('./SeedViewport.jsx', () => ({ default: () => <div data-testid="mock-viewport" /> }))
vi.mock('./SeedGraphSurface.jsx', () => ({
    default: (props) => (
...
import SeedEditor from './SeedEditor.jsx'
...
describe('SeedEditor undo/redo', () => {
    it('Ctrl+Z does not throw when history is empty', () => {
        render(<SeedEditor localStorageKey="test-undo" />)
        expect(() => {
    ...
    it('Ctrl+Y does not throw when redo stack is empty', () => {
        render(<SeedEditor localStorageKey="test-redo" />)
        expect(() => {
    ...
    it('ignores Ctrl+Z when focus is inside a text input', () => {
        const { container } = render(<SeedEditor localStorageKey="test-undo-input" />)
    ...
    it('undo replays inverse ops through the ops path — never replaceDocument, never a local-only dispatch', () => {
        ...
        render(<SeedEditor projectId="proj-1" />)
...
describe('SeedEditor canvas mode', () => {
    ...
    it('does not auto-enter a pre-existing Node 0 — it sits as an ordinary top-level node', () => {
        ...
        render(<SeedEditor localStorageKey={CANVAS_STORAGE_KEY} canvasMode />)
...
describe('SeedEditor delete/reset confirmations', () => {
    ...
    it('deletes Node 0 via the Delete FAB with no confirmation, same as any other node', () => {
        ...
        mockApplyLocalOps.mockClear()
        render(<SeedEditor localStorageKey={GUARD_STORAGE_KEY} />)
...
describe('SeedEditor live-world toggle', () => {
    ...
    it('marks a world live for its scope via setWorkspaceState, keyed by parentId', () => {
        ...
        mockApplyLocalOps.mockClear()
        render(<SeedEditor localStorageKey={LIVE_STORAGE_KEY} />)
...
describe('SeedEditor per-universe chrome visibility', () => {
    ...
    it('shows the full topbar at the true document root', () => {
        ...
        render(<SeedEditor localStorageKey={CHROME_STORAGE_KEY} />)
...
describe('SeedEditor free-nesting palette create', () => {
    ...
    it('creates a second World in the same scope without any block or warning', () => {
        ...
        mockApplyLocalOps.mockClear()
        render(<SeedEditor localStorageKey={FREE_NEST_STORAGE_KEY} />)
