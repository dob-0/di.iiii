...
import { getSeedWorldBackgroundColor, pickActiveTypeNode } from './viewportWorldState.js'
...
describe('getSeedWorldBackgroundColor', () => {
    it('uses the world.background node color before legacy worldState color', () => {
        expect(getSeedWorldBackgroundColor({
    ...
    it('falls back to worldState and then the Seed default', () => {
        expect(getSeedWorldBackgroundColor({
            worldState: { backgroundColor: '#05070a' },
            nodes: []
        })).toBe('#05070a')

        expect(getSeedWorldBackgroundColor({ nodes: [] })).toBe('#0a0e16')
    })
    ...
    it('resolves a graph-driven background color', () => {
        ...
        expect(getSeedWorldBackgroundColor(document, createNodeGraphContext(document))).toBe('#112233')
    })
    ...
    it('only matches a world.background node in the same scope when scopeId is given', () => {
        ...
        expect(getSeedWorldBackgroundColor(document, null, { scopeId: 'my-world' })).toBe('#05070a')
        ...
        expect(getSeedWorldBackgroundColor(documentWithMatch, null, { scopeId: 'my-world' })).toBe('#00ff00')
    })
    ...
    it('falls back to the world node\'s own bgColor before worldState, once scoped', () => {
        ...
        expect(getSeedWorldBackgroundColor(document, null, { scopeId: 'root', worldNode })).toBe('#663399')
    })
    ...
    it('with multiple world.background siblings in one scope, uses the one marked active', () => {
        ...
        expect(getSeedWorldBackgroundColor(document, null, { scopeId: 'my-world' })).toBe('#222222')
    })
})
