...
export const PROJECTOR_LIVE_HTML = `<!doctype html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Live Projector</title>
  <style>
...
<body>
  ...
  <div id="overlay">waiting for cues…</div>

  <script>
    var scene = document.getElementById('scene');
    var overlay = document.getElementById('overlay');
    var slides = ['', 'act 1', 'act 2', 'act 3', 'fin'];
    var current = 0;
    ...
    function show(text, color) {
      ...
      setTimeout(function() {
        ...
        if (color) scene.style.color = color;
        scene.style.opacity = '1';
      }, 200);
    }

    window.studio.onCue('next', function() {
      current = Math.min(current + 1, slides.length - 1);
      show(slides[current]);
      overlay.textContent = 'cue: next → slide ' + current;
    });

    window.studio.onCue('prev', function() {
      current = Math.max(current - 1, 0);
      show(slides[current]);
      overlay.textContent = 'cue: prev → slide ' + current;
    });

    window.studio.onCue('blackout', function() {
      show('', '#000');
      document.body.style.background = '#000';
      overlay.textContent = 'cue: blackout';
    });

    window.studio.onCue('whiteout', function() {
      show('', '#fff');
      document.body.style.background = '#fff';
      overlay.textContent = 'cue: whiteout';
    });

    window.studio.onScene(function(state) {
      if (!state || !state.camera) return;
      var pos = state.camera.position;
      if (pos) {
        overlay.textContent = 'cam ' + [pos[0], pos[1], pos[2]].map(function(v) {
          return Number(v).toFixed(1);
        }).join(' / ');
      }
    });

    window.studio.onAny(function(msg) {
      if (msg.type === 'cue' && !['next','prev','blackout','whiteout'].includes(msg.name)) {
        show(msg.name);
        overlay.textContent = 'cue: ' + msg.name;
      }
    });
  </script>
</body>
...
export const CAMERA_DATA_HTML = `<!doctype html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Live Scene Data</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body {
      width: 100vw; height: 100vh; overflow: hidden;
      background: #03060a; color: #6ee7ff;
      font-family: 'JetBrains Mono', 'SFMono-Regular', Consolas, monospace;
    ...
    .block h3 { font-size: 0.65rem; letter-spacing: 0.18em; text-transform: uppercase;
      color: rgba(110,231,255,0.35); margin-bottom: 0.5rem; }
    .row { display: flex; justify-content: space-between; gap: 1rem;
      border-bottom: 1px solid rgba(110,231,255,0.06); padding: 0.2rem 0; }
    .row:last-child { border-bottom: 0; }
    .key { color: rgba(110,231,255,0.5); }
    .val { color: #6ee7ff; }
    #cue-log { font-size: 0.75rem; color: rgba(110,231,255,0.6); padding: 0.5rem 0; }
  </style>
</head>
<body>
  <header>di.studio — live scene stream</header>
  <div id="data"><div class="block"><h3>Status</h3><div class="row"><span class="key">signal</span><span class="val" id="status">waiting…</span></div></div></div>
  ...
  <script>
    ...
    function fmt(v) {
      if (Array.isArray(v)) return v.map(function(n) { return Number(n).toFixed(2); }).join(', ');
    ...
    function makeBlock(title, rows) {
      ...
      Object.entries(rows).forEach(function(entry) {
        ...
        row.className = 'row';
        row.innerHTML = '<span class="key">' + entry[0] + '</span><span class="val">' + fmt(entry[1]) + '</span>';
        div.appendChild(row);
      });
      return div;
    }
    ...
    window.studio.onScene(function(state) {
      ...
      data.innerHTML = '';
      if (state.camera) {
        data.appendChild(makeBlock('camera', {
          position: state.camera.position,
          target: state.camera.target
        }));
      }
      (state.entities || []).forEach(function(e) {
        data.appendChild(makeBlock(e.label || e.type || e.id, {
          id: e.id,
          type: e.type,
          position: e.position,
          rotation: e.rotation,
    ...
    window.studio.onCue(function(msg) {
      statusEl.textContent = 'cue: ' + msg.name;
    });

    window.studio.onAny(function(msg) {
      statusEl.textContent = msg.type + (msg.name ? ':' + msg.name : '');
    });
  </script>
</body>
</html>`
...
export const presentationStarterTemplates = [
    ...
    },
    {
        ...
        description: 'Fullscreen cue-driven slides for projection. Fire next / prev / blackout / whiteout from the Present panel.',
        html: PROJECTOR_LIVE_HTML
    },
    {
        id: 'camera-data',
        name: 'Scene Data View',
        eyebrow: 'Live signals',
        description: 'Live readout of camera position and entity transforms streamed from the studio.',
        html: CAMERA_DATA_HTML
    }
]
