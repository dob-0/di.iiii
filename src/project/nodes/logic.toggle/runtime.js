...
export const computeOutput = (node, portId, { input, context, pulsed = () => false }) => {
    if (portId !== 'out') return undefined
    const memory = context?.frameMemory
    if (!memory) return false
    const key = `${node.id}:on`
    if (risingEdge(memory, `${node.id}:reset`, input('reset'), pulsed('reset'))) memory.set(key, false)
    if (risingEdge(memory, `${node.id}:edge`, input('flip'), pulsed('flip'))) memory.set(key, !memory.get(key))
