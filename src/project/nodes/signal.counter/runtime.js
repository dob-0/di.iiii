...
export const computeOutput = (node, portId, { input, asNumber, context, pulsed = () => false }) => {
    if (portId !== 'out') return undefined
    const memory = context?.frameMemory
    if (!memory) return 0
    const key = `${node.id}:count`
    if (risingEdge(memory, `${node.id}:reset`, input('reset'), pulsed('reset'))) memory.set(key, 0)
    if (risingEdge(memory, `${node.id}:edge`, input('count'), pulsed('count'))) {
