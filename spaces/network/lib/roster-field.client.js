...
function createRosterField(svg, roster, works) {
  ...
  const LABEL = 104;   // room a work's name needs beside the outermost spine
  let rows = [];
  ...
  function build() {
    ...
    // No margin, no drawing: the brackets sit outside the list, and a bracket
    // clipped by the window edge is a mark that stops meaning anything.
    const need = GAP + FIRST + Math.max(0, works.length - 1) * STEP + 14 + LABEL;
    if (window.innerWidth - rowRight < need) { svg.style.display = 'none'; return; }
    svg.style.display = '';
